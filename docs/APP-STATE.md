...
- **Social + a11y:** **winner-spotlight carousel** (rotates real on-chain winners) on the pitch page; **`/embed/pot`** chrome-less iframe widget (+ copy-snippet on /stats); accessible **"?" tooltips** on weight/odds/Growing-Pot/cooldown; full **a11y pass** (focus rings, aria-labels, heading hierarchy, WCAG-AA contrast in both themes, motion gating).
...
_(✅ also 2026-06-07: FAQ; OG share image + /share; interactive explainer; badge progress; odds-projection; PWA; winner carousel + /embed/pot; "?" tooltips + a11y/contrast pass.)_
**Remaining no-contract (minor / diminishing returns):** custom themed connect modal; streak reminders/notifications; deeper personal-stats history; standalone referral-dashboard page.
**Decision-gated (higher leverage — needs your accounts):** Vercel + Supabase + fresh launch pool = go-live for the demand probe.
**Contract-depth (needs your sign-off — touches the audit-direction program):** auto-scheduler for cron draws; pool-scoped `UserPosition` (fixes the leaderboard pollution at the root); real on-chain referral odds-boost.
